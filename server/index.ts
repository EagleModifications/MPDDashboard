...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      if (!code || !state) {
        return res.redirect(
          "http://localhost:5173/signed-out?error=oauth",
        )
      }
      ...
      try {
        ...
        if (
          ...
          return res.redirect(
            "http://localhost:5173/signed-out?error=state",
          )
        }
      } catch {
        return res.redirect(
      ...
      if (!tokenResponse.ok) {
        ...
        return res.redirect(
          "http://localhost:5173/signed-out?error=discord",
        )
      }
      ...
      if (!rosterUser) {
        ...
        return res.redirect(
          "http://localhost:5173/signed-out?error=not_registered",
        )
      }
      ...
      if (
        ...
        return res.redirect(
          "http://localhost:5173/signed-out?error=inactive",
        )
      }
      ...
      return res.redirect(
        "http://localhost:5173/signed-out?error=server",
      )
    }
  },
)
...
app.get(
  ...
  (_req, res) => {
    ...
    res.redirect(
      "http://localhost:5173/signed-out?logout=true",
    )
  },
)
